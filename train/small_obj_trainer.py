...
from dataset.dataset import VisDroneDataset
...
def train(args, params):
    ...
    model = nn.load_model("/home/quang/CODE/SimpFSOD/yoloe-v8s-pretrained.pt", 11)
    ...
    # Dataset
    dataset = VisDroneDataset("/home/quang/DATA/VisDrone2019-DET-train", 640, params, augment=True, nc = 11)
    loader = data.DataLoader(dataset, args.batch_size, True, num_workers=8, pin_memory=True, collate_fn=VisDroneDataset.collate_fn)
    
    dataset_test = VisDroneDataset("/home/quang/DATA/VisDrone2019-DET-test", 640, params, augment=False, nc = 11)
    loader_test = data.DataLoader(dataset_test, batch_size=4, shuffle=False, num_workers=4, pin_memory=True, collate_fn=VisDroneDataset.collate_fn)
    ...
    for epoch in range(args.epochs):
        ...
        for i, (sample, prompt_mask, targets) in p_bar:
            ...
            prompt_mask = prompt_mask.cuda().float()
...
@torch.no_grad()
def test(args, params, model, loader):
    # model.half()
    model.eval()
    ...
    for samples, prompt_mask, targets in p_bar:
        sample = samples.cuda().float() / 255
        prompt_mask = prompt_mask.cuda().float()
